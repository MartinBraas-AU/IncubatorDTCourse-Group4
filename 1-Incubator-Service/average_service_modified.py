...
class AverageService(RPCServer):
    ...
    def setup(self):
        ...
        """
        super(AverageService, self).setup(routing_key='incubator.dtcourse.average_service_modified', queue_name='incubator.dtcourse.average_service_modified')
    ...
    def compute_std_error(self, values):
        """ 
        This is a method that calculate the standard error of the mean of a list of values. The standard error of the mean is calculated as the standard deviation of the values divided by the square root of the number of values. This gives us an estimate of how much the average would vary if we were to repeat the experiment multiple times.
        """
        if len(values) > 0:
            std_dev = (sum((x - self.compute_average(values)) ** 2 for x in values) / len(values)) ** 0.5
            std_error = std_dev / (len(values) ** 0.5)
            return std_error
        else:
            self._l.warning("Received an empty list of values. Cannot compute standard error. Returning error")
            return -1.0

    def compute(self, values, reply_fun):
        """
        This is a method that calculate the average and standard error of the mean of a list of values. The standard error of the mean is calculated as the standard deviation of the values divided by the square root of the number of values. This gives us an estimate of how much the average would vary if we were to repeat the experiment multiple times.
        """

        average = self.compute_average(values)
        std_error = self.compute_std_error(values)
