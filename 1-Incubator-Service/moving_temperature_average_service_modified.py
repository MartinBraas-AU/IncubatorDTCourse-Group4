...
class MovingAverageTemperatureService(RPCServer):
    ...
    def setup(self):
        ...
        """
        super(MovingAverageTemperatureService, self).setup(routing_key='incubator.dtcourse.moving_temperature_average_service_modified', queue_name='incubator.dtcourse.moving_temperature_average_service_modified')
    ...
    def process_state_sample(self, ch, method, properties, body_json):
        ...
        response = self._rpc_client.invoke_method("incubator.dtcourse.average_service_modified", "compute", arguments)

        # Log the response
        self._l.info(f"average_service response: {response}")

        if "average" and "std_error" in response:
