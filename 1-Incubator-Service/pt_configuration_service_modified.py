...
class PTReconfigurationService:
    ...
    def __init__(self, max_diff, new_temp_desired, rabbitmq_config):
        ...
        self._confirm_samples = 2  # start with 2, tune upward
        self._anom_streak = 0
        self._last_reconfig_time = 0.0
        self._cooldown_seconds = 1.0  # prevents spamming reconfig
    ...
    def check_if_reconfiguration_needed(self):
        ...
        if diff <= self._max_diff:
            if self._anom_streak != 0:
                self._l.info(f"Anomaly streak reset (diff back to normal).")
            self._anom_streak = 0
        ...
        self._anom_streak += 1
        self._l.info(
            f"Potential anomaly ({self._anom_streak}/{self._confirm_samples}). "
            f"Temp: {self.temp}, Avg: {self.avg}, Difference: {diff}"
        )

        if self._anom_streak < self._confirm_samples:
            # Not enough evidence yet
            return

        # Optional cooldown to avoid repeated reconfigs
        now = time.time()
        if now - self._last_reconfig_time < self._cooldown_seconds:
            self._l.info(
                f"Confirmed anomaly but in cooldown ({now - self._last_reconfig_time:.1f}s < {self._cooldown_seconds}s). "
                f"Skipping reconfiguration."
            )
            self._anom_streak = 0
            return

        self._l.info(
            f"Confirmed anomaly after {self._confirm_samples} samples. Triggering reconfiguration."
        )
